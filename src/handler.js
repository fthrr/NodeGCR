...
const getAllBooksHandler = (request, h) => {
  ...
  if (reading !== undefined) {
    const readingBook = books.filter((book) => book.reading === !!Number(reading));
    const response = h.response({
      status: 'success',
      data: {
  ...
  if (finished !== undefined) {
    const finishedBook = books.filter((book) => book.finished === !!Number(finished));
    const response = h.response({
      status: 'success',
      data: {
...
const getDetailedBookByIdHandler = (request, h) => {
  ...
  const book = books.filter((detailBook) => detailBook.id === bookId)[0];
